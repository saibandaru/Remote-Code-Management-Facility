...
void Server::sender()
{
	try	{
		...
		while (true){
			int size = q1.size();popQ = q1.deQ();pastportR = portR;
			...
			if (commandI == "DOWNLOAD")	{
				//if (pastportR != portR){//	//si.shutDownSend();//	//}
				connectadd(si, addressR, portR);
				CT.fileUpload(si, filename, senderI, commandI,path); 
			}
			else if (commandI == "UPLOAD" || commandI == "UPLOADFAIL"){
				/*if (pastportR != portR){si.shutDownSend();connectadd(si, addressR, portR);
				}*/connectadd(si, addressR, portR);
				CT.uploadfileAck(si, filename, senderI, commandI);
			}
			else if (commandI == "TEXTSEARCH"){
				/*if (pastportR != portR){si.shutDownSend();connectadd(si, addressR, portR);
				}*/connectadd(si, addressR, portR);
			CT.searchText(si, filename, senderI, commandI);
			}
			else if (commandI == "FILESEARCH"){
				/*if (pastportR != portR){	si.shutDownSend();	connectadd(si, addressR, portR);
				}*/connectadd(si, addressR, portR);
			CT.searchFile(si, filename, senderI, commandI);
			}
